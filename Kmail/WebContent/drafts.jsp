...
<jsp:useBean id="drafts" class="Kmail.Drafts"/>
...
<title>Drafts-<%=session.getAttribute("userId")%>@kmail.com</title>
...
<table width="100%">
  ...
  <tr>
    ...
    <tr>
    	...
    	<td>
        	<!--<input type="button" onclick="javascript:deleteIt();"/ value="Delete">-->
	...
	<tr>
        ...
        <table width="100%" align="center" bordercolor="#0000FF" cellspacing="0">
               ...
               <%
        		...
        		drafts.setData(userId);
        	    msgId=drafts.getMessageId();
        	    receiver=drafts.getReceiver();
        	    subject=drafts.getSubject();
        	    msgDateTime=drafts.getMsgDateTime();
			    
			    if(msgId.size()==0)
			    {%>
			    	<tr bgcolor="#666666">
    				<td colspan="4" align="center">
    					<font color="#FF0000" face="Georgia, Times New Roman, Times, serif">Draft Is Empty</font>
    				</td>
    				</tr>
			   <% }
			    else
			    {
			    	for(int i=msgId.size()-1;i>=0;i--)
			    	{
			    			...
			    			<tr bgcolor="#666666" class="lovelyrow">
                                ...
                                <td width="20%" onclick="location.href='ReadDraft.jsp?msgId=<%=msgId.get(i)%>&sender=<%=session.getAttribute("userId")%>&subject=<%=subject.get(i)%>'">
			    					<%=receiver.get(i)%>
			    				</td>
                                
                                <td width="55%" onclick="location.href='ReadDraft.jsp?msgId=<%=msgId.get(i)%>&sender=<%=session.getAttribute("userId")%>&subject=<%=subject.get(i)%>'">
                                	<%=subject.get(i)%>
                                </td>
                                
                                <td width="25%" onclick="location.href='ReadDraft.jsp?msgId=<%=msgId.get(i)%>&sender=<%=session.getAttribute("userId")%>&subject=<%=subject.get(i)%>'">
